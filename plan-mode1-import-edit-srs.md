...
8. **Bug FLF-179** (người dùng gặp khi upload SRS thật 2026-09-19): bảng có ô tiêu đề cột trống ⇒ `TemplateProfile validation failed: table_map.N.header is required`. Sửa: model cho `header` rỗng; FE hiện "Cột N (không có tiêu đề)". Nhánh `bugfix/FLF-179-mode1-empty-table-header` ở BE (`cd4f7b3`, tách từ nhánh FLF-178) và FE (`bac23df`, tách từ nhánh P4 FE), chưa push.
