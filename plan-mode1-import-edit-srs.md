...
7. **P4 xong 2026-09-19** (DoD 3/4): BE 1376 test xanh + 3 `it.fails`, coverage dòng 4 module 97–100%; FE 426 test xanh. 2 lỗi mức trung bình đã sửa ở Bug **FLF-178**, BE nhánh `bugfix/FLF-178-mode1-cr-write-table-block-id` (từ nhánh P4, 2 commit, chưa push) — `reports/mode1-p4-report.md` §4.1; còn 4 lỗi thấp. Tiếp theo: push nhánh P3 / e2e-fixes / P4 / FLF-178 và mở PR để chạy CI (DoD cuối), comment PR vào FLF-178 + chuyển **In Review**.
8. **Bug FLF-179** (người dùng gặp khi upload SRS thật 2026-09-19): bảng có ô tiêu đề cột trống ⇒ `TemplateProfile validation failed: table_map.N.header is required`. Sửa: model cho `header` rỗng; FE hiện "Cột N (không có tiêu đề)". Cùng ticket: finalize lỗi `Phần tử id "F-01" đã tồn tại trong "features[]"` — feature "General" (gom màn/function mồ côi) nhận trùng id feature trích được ⇒ allocator giữ chỗ id trong lô. Nhánh `bugfix/FLF-179-mode1-empty-table-header` ở BE (`cd4f7b3`, `a77251d`, tách từ nhánh FLF-178) và FE (`bac23df`, tách từ nhánh P4 FE), chưa push.
...
### DoD P3
- [x] Đi trọn luồng §1 trên trình duyệt với BE thật (không mock): tạo project mode 1 → import → gap report → CR → duyệt một phần → tải `0.1` → release `1.0` → tải bản sạch. *(2026-09-19, `e2e/mode1.spec.ts`, báo cáo P3 §6.1)*
- [x] Mọi trạng thái paused/lỗi AI có banner và nút tiếp tục; không có màn trắng khi API lỗi. *(kiểm trên mock)*
- [x] Workspace mode 2 (luồng cũ) không đổi hành vi: project cũ vẫn mở đúng. *(e2e `workspace.spec.ts` trên BE thật 2/2 xanh)*
