...
| L3 | "Tìm vị trí ảnh hưởng & khoá" bấm không có gì (CR từ gap report) | C-2 trả đích là **mã section** (`fixed:3.1.1`…) + từ khoá là tiêu đề mục; C-3 bỏ im lặng ⇒ 0 vị trí, CR đứng ở `impact_review`. Gốc: gap report mời tạo CR cho **mục còn thiếu** — mục trống không có phần tử để sửa (theo D6 phải chạy step) | C-3 nhận đích mã section (mọi phần tử section đó); 0 vị trí ⇒ `409 CR_NO_LOCATIONS` kèm `empty_sections[{section_id,title,step_id}]`; FE: gap report tách khối "mục thiếu → chạy step", prefill CR bỏ `section_empty`/mục thiếu, thông báo lỗi chỉ step. **Phương án B đã làm xong 2026-09-20** — đích là section trống ⇒ vị trí là cả mảng nuôi mục đó (`SECTION_FILL_ARRAYS`), C-4 chỉ được `add` vào đúng mảng (C-5 chặn `op_outside_section`), khoá theo path mảng; section không có mảng nào nuôi vẫn ra `CR_NO_LOCATIONS`. Contract §4.7 |
...
| L7 | "Hiện 4 cờ đỏ nhưng nhấn vào không thấy" | Cột kế hoạch mode 1 chỉ in **số** ("Còn 4 cờ đỏ…"), không liệt kê; bảng cờ nằm ở panel Verification của mode 2 (nút trên thanh phase) mà người dùng mode 1 không biết. API `GET /flags` trả đúng 4 cờ | FE `Mode1PlanPanel` thêm mục "Cờ đỏ đang chặn ký v1 (N)": từng cờ + `rule_id` + nút **Chạy `<step>`**, kèm câu nhắc cờ vàng không chặn và luật nào không waive được |
| L8 | `NOT_IMPLEMENTED: 429 "Plan credits cannot be applied…"` | Endpoint GLM trên Modal từ chối (chưa gắn thanh toán / hết hạn mức). Mã lỗi provider không có trong bảng pipeline ⇒ quy hết về `NOT_IMPLEMENTED` (501), giấu nguyên nhân; 429 lại bị coi là lỗi tạm ⇒ gọi lại 3 lần vô ích | Bảng pipeline thêm `RATE_LIMIT_EXCEEDED` (429) + `AI_PROVIDER_ERROR` (502); `toPipelineErrorCode` map theo status; `isTransientError` không retry khi thông điệp nói về thanh toán/hạn mức; FE có câu tiếng Việt. **Phần tài khoản Modal người dùng tự xử** |
| L9 | Quay về bước cũ rồi bấm "Chạy bước này" ⇒ ra bản accept của **bước sau**, không back được | Nút luôn chạy `current_step` chứ không phải bước đang xem; BE chặn chạy lại step đã accepted (phải gate revision/regenerate) nhưng UI không chỉ đường | Nút đổi thành **"▶ Chạy `<bước đang xem>`"**, chỉ hiện khi bước đó chạy được (chưa accepted/skipped); thêm nút **"↩ Về `<bước hiện tại>`"** |
| S-5 | Thanh step dội 310 bước, không cái nào "Thiếu" | S-5 là vòng theo màn: 61 màn (đều `placeholder`, 0 function gắn màn) + nonscreen ⇒ 5 × 62. `step_plan` cố ý lọc bỏ step loại `loop` nên không bao giờ có nhãn Thiếu. Mẫu số tiến độ cũng đếm cả 305 bước không ai chạy ("38/366") | Theo quyết định người dùng: vòng của màn chưa động tới **gom thành một chip "+N màn để lại"** (vẫn cho thấy ở đó có step, mở khi màn có function); `progressByStep` bỏ màn `placeholder` khỏi mẫu số (mode 2 cũng đúng hơn: trước đây 100% là không thể đạt) |
| L10 | Step S-4.3 "đã chốt" nhưng cờ đỏ `section_empty` vẫn treo, bấm chạy lại thì BE từ chối | Hai tiêu chí lệch nhau: `buildStepPlan` đánh step `accepted` khi **file có chữ** dưới đầu mục, còn cờ đỏ soi **dữ liệu Spine**. File có "Screen Authorization" nhưng I-4 không trích ra role/permission nào ⇒ Spine trống (roles 0, permissions 0) mà step vẫn accepted ⇒ ngõ cụt: cờ bảo chạy S-4.3, mà step accepted thì `/run` chặn | BE: `buildStepPlan` nhận Spine, mục nào luật cờ soi được thì "đã có nội dung" **tính theo dữ liệu Spine** (`sectionHasData`, dùng chung `SECTION_HAS_DATA`); `/run` nhận `reopen: true` để mở lại step đã chốt (đặt `revision_requested`, reset seq — đúng như gate revision, B7). FE: cờ đỏ trỏ tới step đã chốt ⇒ nút **"Mở lại S-4.3"**. Contract pipeline §1 endpoint 4 |
